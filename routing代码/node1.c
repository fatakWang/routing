...
void rtinit1() {
	...
	neighbor1 = getNeighborCosts(NODEID);
	numNodes = neighbor1->NodesInNetwork;
	...
	for (i = 0; i < numNodes; i++) {
		for (j = 0; j < numNodes; j++) {
			if (i == j) { // directly connected neighbor
				...
				min_vertic[i]=neighbor1->NodeCosts[i];
			}
			else {
	...
	printdt1(NODEID, getNeighborCosts(NODEID), &dt1);
	...
	for (i = 0; i < numNodes; i++) {
		if (i != NODEID && neighbor1->NodeCosts[i] < INFINITY) {
			if (TraceLevel >= 3) printf("making packet %d...\n", i);
			toSend.sourceid = NODEID;
			toSend.destid = i;
			for (j = 0; j < numNodes; j++) {
				toSend.mincost[j] = dt1.costs[j][j];
			}
...
/* rtupdate1(struct RoutePacket *rcvdpkt)
 ...
 * packet that was received. rtupdate1() is the heart of the distance vector
 ...
 * rtupdate1() uses these received values to update its own distance table. If
...
void rtupdate1( struct RoutePacket *rcvdpkt ) {
	...
	for (i = 0; i < numNodes; i++) {
		if (dt1.costs[i][src] > dt1.costs[src][src] + rcvdpkt->mincost[i]) {
			dt1.costs[i][src] = dt1.costs[src][src] + rcvdpkt->mincost[i];
			tableUpdated=1;
		}
	}
	if(tableUpdated){
		printf("node%d distance table updated: \n", NODEID);
		printdt1(NODEID, neighbor1, &dt1);
	}
	...
	for (i = 0; i < numNodes; i++) {
		for (j = 0; j < numNodes; j++) {
			...
			if (min_vertic[i] > dt1.costs[i][j]){
				min_vertic[i] = dt1.costs[i][j];
				costupdated=1;
			}
		}
		// printf("\n");
...
// printf("\n");
	if (costupdated) {
		...
		for (n = 0; n < numNodes ; n++) {//
			if(n==NODEID||neighbor1->NodeCosts[n]==INFINITY)continue;
			toSend.destid = n;
			if (TraceLevel >= 2) {
